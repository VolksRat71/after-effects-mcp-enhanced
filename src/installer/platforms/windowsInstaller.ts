...
export class WindowsInstaller {
     ...
     */
    static async install(source: string, aePaths: { scriptUIFolder: string }) {
     ...
     */
    static tryElevatedCopy(source: string, destination: string): boolean {
        try {
            // Normalize paths for PowerShell
            const psSource = source.replace(/\\/g, '\\\\').replace(/'/g, "''");
            const psDestination = destination.replace(/\\/g, '\\\\').replace(/'/g, "''");

            // Use PowerShell with proper quoting and -Verb RunAs for elevation
            const psCommand = `Start-Process powershell -Verb RunAs -Wait -ArgumentList '-NoProfile','-Command','Copy-Item -Path ''${psSource}'' -Destination ''${psDestination}'' -Force'`;
     ...
     */
    static showPostInstallInstructions() {

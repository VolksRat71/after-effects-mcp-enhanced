...
export class ScriptExecutor {
  ...
  runExtendScript(scriptPath: string, args: Record<string, any> = {}): string {
    try {
      console.error(`[MCP ScriptExecutor] Running script: ${scriptPath}`);
      console.error(`[MCP ScriptExecutor] Script args: ${JSON.stringify(args).substring(0, 200)}${JSON.stringify(args).length > 200 ? '...' : ''}`);
      ...
      if (!aePath || !fs.existsSync(aePath)) {
        ...
        console.error(`[MCP ScriptExecutor] ${errorMsg}`);
        return errorMsg;
      }
      ...
      console.error(`[MCP ScriptExecutor] Running command with -m flag: ${command}`);

      try {
        const output = execSync(command, { encoding: 'utf8', timeout: 30000 });
        console.error(`[MCP ScriptExecutor] Script executed successfully`);
        return output;
      } catch (execError: any) {
        console.error("[MCP ScriptExecutor] Command execution error:", execError);
        ...
        console.error("[MCP ScriptExecutor] Trying alternative approach using BridgeTalk...");
...
alert("Script execution completed");
      ...
      console.error("[MCP ScriptExecutor] Error running ExtendScript:", error);
      return errorMsg;
    }
  }
  ...
  executeCustomScript(script: string, description?: string): { tempScriptPath: string; wrappedScript: string } {
    console.error(`[MCP ScriptExecutor] Preparing custom script, description: ${description || 'none'}`);
    console.error(`[MCP ScriptExecutor] Script length: ${script.length} characters`);
...
`;
    ...
    console.error(`[MCP ScriptExecutor] Custom script written to: ${tempScriptPath}`);
  ...
  createTestAnimationScript(operation: string, compIndex: number, layerIndex: number): string {
    ...
    console.error(`[MCP ScriptExecutor] Test script created: ${tempFile}`);
